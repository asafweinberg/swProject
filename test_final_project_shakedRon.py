...
def generate_correct_files_general():
    ...
    exec = ["python spkmeans.py", os.path.join(".", C_file_name)]
    ...
    for row in filesToCreate.itertuples():
        ...
        for goal in goals: 
            ...
            for k in k_values:
                for ex in exec:
                    lng = "P" if ex == "python spkmeans.py" else "C"
        ...
        for ex in exec:
            for goal in goals:
                if goal == "spk":
                    ...
                    k_values = [1]       
                print(ex, k_values, goal, input_data_filename, result_file, input_index)
